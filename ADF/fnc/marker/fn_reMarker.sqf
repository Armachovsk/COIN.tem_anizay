...
// Reporting
if (ADF_extRpt || {ADF_debug}) then {diag_log "ADF rpt: fnc - executing: ADF_fnc_reMarker"};
...
if !(_oldMarker in allMapMarkers) exitWith {
	["ADF_fnc_reMarker", format ["Marker: '%1' is not a valid marker or does not exist", _oldMarker]] call ADF_fnc_terminateScript;
};
...
if !(_shape in ["ICON", "RECTANGLE", "ELLIPSE", "POLYLINE", "EMPTY"]) then {
	...
	_type = "mil_warning";
	_colour = "ColorBlack";
	_text = "ADF_fnc_reMarker: Marker Error (shape)";
};
...
// Debug reporting
if ADF_debug then {[format ["ADF_fnc_reMarker marker (%1) re-created", _name]] call ADF_fnc_log};
