...
**********************************************************************************
ABOUT
Creates a global marker, visibible by everyone

INSTRUCTIONS:
Call from script on the server or hc. To create a marker which is visible on the
map you need to define at least the name, position, shape and type.
...
["MyPosition", getPos player] call ADF_fnc_createMarker;
...
// Reporting
if (ADF_extRpt || {ADF_debug}) then {diag_log "ADF rpt: fnc - executing: ADF_fnc_createMarker"};
...
if (_position isEqualTo [0, 0, 0]) exitWith {["ADF_fnc_createMarker", format ["Position: '%1' is not a valid position", _position]] call ADF_fnc_terminateScript;};
if (!(_type isEqualTo "") && {!(_type in _validTypes)}) then {_shape = "ICON"; _type = "hd_dot"; _colour = "ColorUNKNOWN"; _text = "ADF_fnc_createMarker ERROR (type)";};
if (!(_shape isEqualTo "SYSTEM") && {!(_shape in ["ICON", "RECTANGLE", "ELLIPSE", "POLYLINE"])}) then {_shape = "ICON"; _type = "hd_dot"; _colour = "ColorUNKNOWN"; _text = "ADF_fnc_createMarker ERROR (shape)";};

private _marker = createMarker [_name, _position];
if !(_shape isEqualTo "SYSTEM") then {
	_marker setMarkerShape _shape;
	if !(_type == "") then {_marker setMarkerType _type};
