...
// Reporting
if (ADF_extRpt || {ADF_debug}) then {diag_log "ADF rpt: fnc - executing: ADF_fnc_MHQ"};
...
ADF_fnc_MHQ_PlayerRespawnPos = { 
	...
	"respawn_west" setMarkerPos [(_p # 0) + 3, (_p # 1) + 3, 0];				
};
...
ADF_fnc_MHQ_respawn = {
	...
	if isServer then {
		// Create a new MHQ
		MHQ = createVehicle [ADF_MHQ_vehicle, [0,0,0], [], 0, "CAN_COLLIDE"];		
	...
	if hasInterface then {
		...
		hintSilent parseText format ["<img size= '6' shadow='false' image='%1'/><br/><br/><t color='#6C7169' size='1.5'>%2 MHQ</t><br/><br/><t color='#A1A4AD'>Mobile HQ respawned at grid:</t><br/><t color='#FFFFFF'>%3 %4</t><br/><br/>", ADF_clanLogo, ADF_clanName, round (ADF_MHQ_orgPos # 0), round (ADF_MHQ_orgPos # 1)];
		sleep 5;
		hintSilent "";
	};
};
...
ADF_fnc_MHQ_FinalSpawn = {
	...
	if (ADF_clanFlag != "") then {
		ADF_respawnFlag = createVehicle ["Flag_White_F", [0,0,0], [], 0, "CAN_COLLIDE"];
		ADF_respawnFlag setPos (getMarkerPos "respawn_west");
		ADF_respawnFlag setFlagTexture ADF_clanFlag;		
	} else {
		ADF_respawnFlag = createVehicle ["Flag_NATO_F", [0,0,0], [], 0, "CAN_COLLIDE"];
		ADF_respawnFlag setPos (getMarkerPos "respawn_west");
	};
