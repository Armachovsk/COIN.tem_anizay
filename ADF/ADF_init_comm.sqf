...
// Reporting
diag_log "ADF rpt: Init - executing: ADF_init_comm.sqf";
...
// Vars
private _unit_role = "";
private _group_old = grpNull;
private _group_new = grpNull;
...
// Roster create loop
{
	if (side _x == side player) then {
		_group_new = group _x;
		_roster_group = _group_new;
		_roster_groupName = groupID (_group_new);
		ADF_roster_userGroup = "";
		_unit_role = " (" + getText(configFile >> "CfgVehicles" >> typeOf(_x) >> "displayName") + ")"; // V1.41B01
		if (ADF_TFAR_preset && ADF_mod_TFAR) then {
			if (_group_new != _group_old) then {
				ADF_roster_userGroup = format ["<br/><font size='16' color='#D7DBD5'>%1</font>  <font color='#9DA698'>[</font><font color='#FF9E05'>%2</font><font color='#9DA698'>][</font><font color='#10D471'>%3</font><font color='#9DA698'>]<br/>", _roster_groupName, ADF_TFAR_LR_freq, ADF_TFAR_SW_freq];
			}
		...
		ADF_roster_line =  ADF_roster_line + ADF_roster_userGroup + ADF_roster_uName + _unit_role + "</font><br/>";
		_group_old = group _x;
	};
...
<font color='#9DA698'>ADF version: 2.26 / Jul 2020</font><br/><br/>
"]];